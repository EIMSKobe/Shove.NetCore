...
namespace Shove.HTML.SgmlReader
{
    ...
    public class SgmlReader : XmlReader
    {
        ...
        void Log(string msg, params string[] args)
        {
            if (ErrorLog != null)
            {
                string err = System.String.Format(msg, args);
        ...
        bool ParseStartTag(char ch)
        {
            ...
            while (ch != Entity.EOF && ch != '>')
            {
                ...
                {
                    //shove Add: 属性名字规范化
                    aname = StandardizationIdentifier(aname);
                    //shove Add End.

                    //shove Change: 不写重复的属性
                    //Attribute a = n.AddAttribute(aname, value, quote, this.folding == CaseFolding.None);
                    Attribute a = n.AddAttribute(aname, value, quote, true);
                    //shove Change End.
        ...
        /// <returns></returns>
        public string StandardizationIdentifier(string str)
        {
            str = str.Trim();
            if (str == "")
                return "CHERY_ADD";
